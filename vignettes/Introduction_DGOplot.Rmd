...
Visualization of gene association to the ontology groups. Layout is adapted from [ahmohamed](https://stackoverflow.com/questions/28715736/how-to-spread-out-community-graph-made-by-using-igraph-package-in-r/28722680#28722680) on gitHub.

```{r, echo = TRUE, warning=TRUE}
net <- DGOnetplot(DGOResult)
net()
net <- DGOnetplot(DGOResult, showCategory = 5)
net()
net <- DGOnetplot(DGOResult, pvalueCutoff = 0.05)
net()
```
